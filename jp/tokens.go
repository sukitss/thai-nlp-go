...
// max-probability path over Japanese runs, greedy elsewhere), with the same
...
// dpTokens emits the DP segmentation of the Japanese run rs[a:rb] as tokens
// with byte offsets, starting at byte offset b; with an unweighted dictionary
// it falls back to greedy longest-match confined to the run (the same
// boundaries as CutDP's fallback). Returns the extended slice and the byte
// offset after the run.
...
// Tokens segments Japanese text with the shared Default dictionary and reports
...
// TokensDP segments Japanese text with the shared dictionary using DAG + DP
// and reports byte offsets (see Segmenter.TokensDP).
