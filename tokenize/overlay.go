...
type OverlayDict struct {
	base    dict.Prefixer
	baseW   dict.Weighter // non-nil when base also supports weighted lookups
	overlay *dict.Trie
...
// dict.Weighter): the same ascending, de-duplicated union of lengths, with each
...
// that only implements dict.Prefixer contributes its lengths with weight 0.
